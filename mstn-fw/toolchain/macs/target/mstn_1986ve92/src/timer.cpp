...
{
	_ASSERT(hardware_timer == MDR_TIMER1 || hardware_timer == MDR_TIMER2 || hardware_timer == MDR_TIMER3);
...
Result Timer::Initialize(uint period, MeasureMode mode, TimerAction * action)
{
	...
	if (s_is_first_time) {
		RST_CLK_PCLKcmd((RST_CLK_PCLK_RST_CLK | RST_CLK_PCLK_TIMER1 | RST_CLK_PCLK_TIMER2 | RST_CLK_PCLK_TIMER3), ENABLE);
...
Result Timer::Start(bool immediate_tick)
{
	...
	TIMER_SetCounter(m_timer->m_handle, immediate_tick ? m_timer->m_period - 1 : 0);
